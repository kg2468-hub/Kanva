...
function initToolbar() {
  ...
  function renderGeneralTools() {
    ...
    const tools = [
      ...
      },
      {
        ...
        label: 'Exportar PNG',
        svg: `<svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                <path d="M12 3v12m0 0l-4-4m4 4 4-4M4 21h16" 
                      stroke="currentColor" stroke-width="1.8" 
                      stroke-linecap="round" stroke-linejoin="round"/>
              </svg>`,
        action: () => {
          showConfirmExport((bg, scale) => {
            window.exportCanvasAsPNG?.(bg, scale);
          });
        }
      },
      {
  ...
  function renderImageTools() {
    ...
    const makeBtn = (key, label, svg, action) => {
      const el = document.createElement('button');
      el.className = 'tool-btn';
      el.innerHTML = `<div class="icon">${svg}</div><div class="label">${label}</div>`;
      el.onclick = () => {
        closeAllPanels();
        hideUIShield();
        action();
    ...
    const tools = [
      {
        ...
        action: () => {
          ...
          openSliderPanel('Opacidade', 0, 1, obj.opacity ?? 1, val => {
            obj.opacity = val;
            window.redrawCanvas?.();
          });
        }
      },
      {
        ...
        label: 'Tamanho',
        svg: `<svg width="24" height="24" viewBox="0 0 24 24" fill="none">
          <path d="M3 3h18v18H3z" stroke="currentColor" stroke-width="1.8"/>
        ...
        action: () => {
          ...
          openSliderPanel('Tamanho da imagem (%)', 1, 700, current, val => {
            // mantém o flip (sinal)
      ...
      
      {
        ...
        svg: `<svg width="24" height="24" viewBox="0 0 24 24" fill="none">
          ...
          <path d="M4 10a8 8 0 0 1 8-8h8" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>
          <path d="M20 14a8 8 0 0 1-8 8H4" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>
        </svg>`,
        action: () => {
          const obj = getActiveObject?.();
          if (!obj || obj.type !== 'Imagem') return showToast('Selecione uma imagem');
          const deg = ((obj.rotationLocal || 0) * 180 / Math.PI);
          openSliderPanel('Rotação (°)', -180, 180, deg, val => {
            obj.rotationLocal = val * Math.PI / 180;
            window.redrawCanvas?.();
          });
        }
      },
      {
        ...
        label: 'Espelhar',
        svg: `<svg width="24" height="24" viewBox="0 0 24 24" fill="none">
          <path d="M12 4v16M4 4h16v16H4z" stroke="currentColor" stroke-width="1.8"/>
        </svg>`,
        action: () => openFlipPanel()
      },
      {
        key: 'duplicate',
        label: 'Duplicar',
        ...
        action: () => {
          const obj = getActiveObject?.();
          if (obj) window.duplicateActiveImage?.();
        }
      },
      {
        key: 'delete',
        ...
        action: () => window.confirmDeleteActiveImage?.()
      }
    ];
  
    tools.forEach(t => toolbar.appendChild(makeBtn(t.key, t.label, t.svg, t.action)));
  }
...
function showToast(msg) {
  ...
  if (!toast) {
    ...
    toast.style.cssText = `
      ...
      transition:opacity .3s;backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px);
    `;
...
// ======================================================
// Modal de Confirmação (Kanva Style)
// ======================================================
// ======================================================
// Modal de Confirmação (Kanva Style) — com escolha de fundo
// ======================================================
// ======================================================
// Modal de Exportação Kanva — Fundo + Qualidade
// ======================================================
function showConfirmExport(onConfirm) {
  ...
  box.innerHTML = `
    <h3 style="margin:0 0 14px;font-size:16px;">Exportar Projeto</h3>

    <!-- Fundo -->
    <p style="font-size:14px;margin:10px 0 8px;">Fundo:</p>
    <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:16px;">
      <label style="display:flex;align-items:center;gap:8px;justify-content:center;cursor:pointer;">
        <input type="radio" name="bgopt" value="transparent" checked>
        <span>Fundo <b>transparente</b> (PNG alfa)</span>
      </label>
      <label style="display:flex;align-items:center;gap:8px;justify-content:center;cursor:pointer;">
        <input type="radio" name="bgopt" value="white">
        <span>Fundo <b>branco</b></span>
      </label>
    </div>

    <!-- Qualidade -->
    <p style="font-size:14px;margin:6px 0 8px;">Qualidade:</p>
    ...
    <div style="display:flex;gap:10px;justify-content:center;">
      <button id="confirm-yes"
        style="padding:8px 16px;background:#2563eb;color:#fff;border:none;
               border-radius:8px;font-weight:600;cursor:pointer;">Exportar</button>
      <button id="confirm-no"
        style="padding:8px 16px;background:rgba(255,255,255,0.15);color:#fff;
               border:none;border-radius:8px;cursor:pointer;">Cancelar</button>
    </div>
  `;
  ...
  const bgRadios = box.querySelectorAll('input[name="bgopt"]');
  const qRadios = box.querySelectorAll('input[name="quality"]');

  btnNo.onclick = () => overlay.remove();
  btnYes.onclick = () => {
    const bg = [...bgRadios].find(r => r.checked)?.value || 'transparent';
    const scale = parseFloat([...qRadios].find(r => r.checked)?.value || '1');
    overlay.remove();
    if (onConfirm) onConfirm(bg, scale);
  };
}
...
function openFlipPanel() {
  ...
  // 🔹 Cria painel fixo igual ao dos sliders
  const panel = document.createElement('div');
  panel.className = 'slider-panel show';
  panel.style.display = 'flex';
  panel.style.flexDirection = 'column';
  panel.style.justifyContent = 'center';
  panel.style.alignItems = 'center';
  panel.style.zIndex = 2001;
  panel.style.padding = '14px 0 18px';
  ...
  panel.innerHTML = `
    ...
    <div class="flip-controls">
      <button class="flip-option" id="flip-h">
      ...
      <button class="flip-option" id="flip-v">
        <svg width="30" height="30" viewBox="0 0 24 24" fill="none">
          <path d="M4 12h16M4 4h16v16H4z" stroke="currentColor" stroke-width="1.8"/>
  ...
  const btnV = document.getElementById('flip-v');

  btnH.onclick = () => {
    obj.scaleX = -(obj.scaleX || obj.scale || 1);
    highlightButton(btnH);
    window.redrawCanvas?.();
  };
  
  btnV.onclick = () => {
    obj.scaleY = -(obj.scaleY || obj.scale || 1);
    highlightButton(btnV);
    window.redrawCanvas?.();
  };
  

  // 🔹 Marca o botão pressionado com feedback
  function highlightButton(el) {
    document.querySelectorAll('.flip-option').forEach(b => b.classList.remove('active'));
    el.classList.add('active');
    setTimeout(() => el.classList.remove('active'), 400);
  }

  // 🔹 Garante que o painel feche se clicarmos fora
