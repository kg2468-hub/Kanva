...
function syncOrderWithCanvas() {
  ...
  // 🔹 Reordena de acordo com as camadas visíveis no painel
  const ordered = layers
    .map(l => objs.find(o => o.id === l.id))
    .filter(Boolean);
  ...
  // 🔹 Garante que os objetos de IMAGEM também sejam redesenhados
  ...
  function buildItem(layer) {
    ...
    el.addEventListener('dragover', e => {
      ...
      if (insertAbove) el.before(dropIndicator);
      else el.after(dropIndicator);
    });
